...
final class RoomCreationIntroCellContentView: UIView, NibLoadable, Themable {
    ...
    func fill(with viewData: RoomCreationIntroViewData) {
        ...
        if viewData.roomDisplayName.hasPrefix("[TG]") {
            ...
            // Append the text and space to the attributed string
            attributedString.append(spaceString)
            attributedString.append(textString)

            // Set the attributed string to the UILabel
            self.titleLabel.attributedText = attributedString
        } else if viewData.roomDisplayName.hasPrefix("$"){
            let roomDisplayNameWithoutTG = viewData.roomDisplayName.replacingOccurrences(of: "$", with: " ")
    ...
    private func buildRoomInformationText(with roomName: String, topic: String?) -> NSAttributedString {
        ...
        let replacedFirstSentencePart2 = replaceTextWithImage(text: firstSentencePart2, targetTexts: ["[TG] ","$"], imageNames: ["chatimg", "dollar"], imageSize: CGSize(width: 15, height: 15))
        ...
        attributedString.append(replacedFirstSentencePart2)
    ...
    private func replaceTextWithImage(text: NSAttributedString, targetTexts: [String], imageNames: [String], imageSize: CGSize) -> NSAttributedString {
        let mutableAttributedString = NSMutableAttributedString(attributedString: text)

        for (index, targetText) in targetTexts.enumerated() {
            if text.string.contains(targetText) {
                let targetRange = (text.string as NSString).range(of: targetText)
                let attachment = NSTextAttachment()
                attachment.image = UIImage(named: imageNames[index])?.resize(targetSize: imageSize)
                let imageString = NSAttributedString(attachment: attachment)
                mutableAttributedString.replaceCharacters(in: targetRange, with: imageString)
            }
        }
